...
import { ArrowUpRight, MapPin, Layers, MessageSquare, ChevronDown, ChevronUp } from "lucide-react";

export default function ProjectsGallery() {
  const [showAll, setShowAll] = useState(false);

  const displayedProjects = showAll ? PORTFOLIO_PROJECTS : PORTFOLIO_PROJECTS.slice(0, 4);

  return (
    <section id="projects" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#0d0f12] relative">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Projects Grid Container with Fade Transition */}
        <div className="relative">
          ...
          {/* Transparent Gradient Fade & "Daha Fazla Göster" Button */}
          {!showAll && PORTFOLIO_PROJECTS.length > 4 && (
            <div className="relative -mt-32 pt-32 pb-4">
              {/* Elegant Gradient Mask */}
              <div className="absolute inset-0 bg-gradient-to-t from-[#0d0f12] via-[#0d0f12]/90 to-transparent pointer-events-none" />

              <div className="relative z-10 flex flex-col items-center justify-center pt-8">
                <button
                  onClick={() => setShowAll(true)}
                  className="inline-flex items-center gap-3 bg-[#181d24] hover:bg-[#ff5e14] border-2 border-[#ff5e14] text-white px-8 py-4 text-xs sm:text-sm uppercase font-black tracking-widest transition-all duration-300 shadow-[0_0_30px_rgba(255,94,20,0.35)] hover:shadow-[0_0_45px_rgba(255,94,20,0.7)] group cursor-pointer"
                >
                  <span>Daha Fazla Proje Göster ({PORTFOLIO_PROJECTS.length - 4} İmalat)</span>
                  <ChevronDown className="w-4 h-4 text-[#ff5e14] group-hover:text-white transition-transform group-hover:translate-y-1" />
                </button>
              </div>
            </div>
          )}
          ...
          {showAll && (
            <div className="flex justify-center mt-12">
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#12151a] hover:bg-[#181d24] border border-[#28303d] hover:border-[#ff5e14] text-[#8b9bb0] hover:text-white text-xs uppercase font-bold tracking-wider transition-colors cursor-pointer"
              >
